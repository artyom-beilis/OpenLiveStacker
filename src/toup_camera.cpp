...
namespace ols {
    static const int TOUPCAM_ERROR_OPEN_BY_INDEX    = -1;
    static const int TOUPCAM_ERROR_OPEN_BY_INFO     = -2;
    static const int TOUPCAM_ERROR_INVALD_OPTION    = -3;
    static std::string make_message(std::string const &msg, int code)
    {
        std::string str;
        if(SUCCEEDED(code)) {
            str="Success";
        ...
        switch(code) {
        case TOUPCAM_ERROR_OPEN_BY_INDEX:   str="no camera connected or index value out of boundary"; break;
        case TOUPCAM_ERROR_OPEN_BY_INFO:    str="Strange error"; break;
        case TOUPCAM_ERROR_INVALD_OPTION:   str="Invalid option"; break;
        default:
    ...
    public:
        ...
        virtual std::vector<CamStreamFormat> formats(CamErrorCode &)
        {
            ...
            for(auto bin: bins_)
            {
                if(bin < 2) continue; // bin1 already in ret
        ...
        virtual void start_stream(CamStreamFormat format,frame_callback_type callback,CamErrorCode &e) 
        {
            ...
            */
            ...
            else if(frameRate > 63) frameRate = 63;
...
#define MAKEFOURCC(a, b, c, d) ((unsigned)(unsigned char)(a) | ((unsigned)(unsigned char)(b) << 8) | ((unsigned)(unsigned char)(c) << 16) | ((unsigned)(unsigned char)(d) << 24))
        ...
        virtual void set_parameter(CamOptionId id,double value,CamErrorCode &e)
        {
            ...
            case opt_average_bin:
                {
                    if(NULL == average_bin_support_) break;
                    average_bin_ = !!value;
                    int binning = 0;
                    hr=Toupcam_get_Option(hcam_, TOUPCAM_OPTION_BINNING, &binning);
                    if(FAILED(hr)) break;
            ...
            case opt_cooler_power_perc:
                {
                    unsigned perCent = (unsigned)(value);
