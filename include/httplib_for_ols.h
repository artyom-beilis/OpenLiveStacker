...
//  MIT License
//
// Modified by Artyom Beilis - namespace trname httplib to httplib_for_ols to prevent symbol collision with 
// indi
...
#error                                                                         \
    "cpp-httplib_for_ols doesn't support 32-bit Windows. Please use a 64-bit compiler."
#elif defined(__SIZEOF_POINTER__) && __SIZEOF_POINTER__ < 8
#warning                                                                       \
    "cpp-httplib_for_ols doesn't support 32-bit platforms. Please use a 64-bit compiler."
#elif defined(__SIZEOF_SIZE_T__) && __SIZEOF_SIZE_T__ < 8
#warning                                                                       \
    "cpp-httplib_for_ols doesn't support platforms where size_t is less than 64 bits."
#endif

#ifdef _WIN32
#if defined(_WIN32_WINNT) && _WIN32_WINNT < 0x0602
#error                                                                         \
    "cpp-httplib_for_ols doesn't support Windows 8 or lower. Please use Windows 10 or later."
#endif
#endif
...
inline bool mmap::open(const char *path) {
  ...
  // See:
  // https://github.com/yhirose/cpp-httplib_for_ols/issues/1903#issuecomment-2316520721
...
inline ReadContentResult read_content_chunked(Stream &strm, T &x,
  ...
  //
  // According to the reference code in RFC 9112, cpp-httplib_for_ols now allows
...
inline ssize_t write_response_line(Stream &strm, int status) {
  ...
  s += " ";
  s += httplib_for_ols::status_message(status);
  s += "\r\n";
  return strm.write(s.data(), s.size());
}
...
inline std::string params_to_query_str(const Params &params) {
  ...
  for (auto it = params.begin(); it != params.end(); ++it) {
    ...
    query += it->first;
    query += "=";
    query += httplib_for_ols::encode_uri_component(it->second);
  }
  return query;
}
...
inline std::string make_multipart_data_boundary() {
  return "--cpp-httplib_for_ols-multipart-data-" + detail::random_string(16);
}
...
inline bool ClientImpl::write_request(Stream &strm, Request &req,
  ...
  if (!req.content_receiver) {
    ...
    if (!req.has_header("User-Agent")) {
      auto agent = std::string("cpp-httplib_for_ols/") + CPPHTTPLIB_VERSION;
